...
import { BookOpen, AlertTriangle, Sparkles, FileText, Image as ImageIcon, Upload, Mic } from 'lucide-react';

const HowToUseView: React.FC = () => {
  return (
    <div className="space-y-10 max-w-6xl mx-auto pb-12 animate-slide-up">
        ...
        <div className="flex gap-4 items-start group">
          ...
          <div className="space-y-3 w-full">
            ...
            <ul className="space-y-3 mt-2">
              {[
                { label: "Write or Paste", desc: "Type the details or paste the suspicious message directly into the text box.", icon: FileText },
                { label: "Add Photo", desc: "Upload a screenshot of the chat, email, or QR code.", icon: ImageIcon },
                { label: "Upload Audio", desc: "Attach a recording of a phone call or voicemail.", icon: Upload },
                { label: "Record Audio", desc: "Tap the microphone to speak and describe what happened in your own words.", icon: Mic }
              ].map((item, idx) => (
                <li key={idx} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-stone-600 dark:text-stone-300 text-base leading-relaxed bg-stone-50 dark:bg-stone-800/50 p-3 rounded-xl border border-stone-100 dark:border-stone-700/50">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-orange-600 dark:text-orange-400 whitespace-nowrap">{item.label}:</span>
                  </div>
                  <span>{item.desc}</span>
                </li>
              ))}
